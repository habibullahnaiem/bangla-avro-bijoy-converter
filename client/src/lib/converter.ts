 ...
 *   । → ' (U+0027)  — সুনতন্নীতে প্রকৃত danDi গ্লিফ (quotesingle টিয়ারড্রপ)
 ...
 * (per-chars পরীক্ষাকৃত) — তাই প্রি-ম্যাপ নিরাপদ। দাঁড়ি-কোড U+0027 সুতন্নীর প্রকৃত দাঁড়ি।
 */
function preMapPunctuation(s: string): string {
  const map: Record<string, string> = {
    "।": "\u0027", // দাঁড়ি — সুতন্নীতে quotesingle = টিয়ারড্রপ দাঁড়ি স্ট্রোক
    "॥": "\u005C\u005C",
    "\u2014": "\u00D1", // em-dash
...
export function convertToUnicode(text: string): string {
  ...
  // লাইব্রেরি ইতোমধ্যে Ò→“ Õ→’ Ñ→— রাউন্ড-ট্রিপ করে; দাঁড়ি-কোড ' (U+0027) লাইব্রেরি
  // অপরিবর্তিত রাখে — তাই শুধু সেটা হাতে বদলাও।
  r = r.replace(/\u005C{2}/g, "॥").replace(/\u005C/g, "॥");
  // দাঁড়ি-রাউন্ড-ট্রিপ: ' (U+0027) শব্দের শেষে/বিরামের আগে হলে দাঁড়ি,
  // শব্দের আগে হলে উদ্ধৃতি-টিক — হিউরিস্টিক প্রসঙ্গ নিয়ে নির্ধারণ।
  r = r.replace(/\u0027(?=[ \t\n\r।,.:;!?\u201C\u201D\u2018\u2019"]*$)/g, "।");
  return r;
}
