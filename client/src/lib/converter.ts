 ...
 *   । → ¥ (U+00A5)  — সুনতন্নীতে danDi সোজা স্ট্রোক (হাতে-বানানো গ্লিফ)
 *   ॥ → \ (U+005C)  — সুনতন্নীতে danDi-danDi ডাবল-ভার্টিকাল গ্লিফ
 ...
 * (per-chars পরীক্ষাকৃত) — তাই প্রি-ম্যাপ নিরাপদ। দাঁড়ি-কোড U+00A5 সুতন্নীতে
 * হাতে-বানানো danDi স্ট্রোক গ্লিফ (ইম্বেডেড ওয়েবফন্টে U+00A5 → danDi),
 * তাই প্রিভিউ ও কনভার্টেড টেক্সট উভয়ে দাঁড়ি সঠিক সোজা স্ট্রোকে দেখায়।
 */
function preMapPunctuation(s: string): string {
  const map: Record<string, string> = {
    "।": "\u00A5", // দাঁড়ি — সুতন্নীতে danDi সোজা স্ট্রোক (হাতে-বানানো গ্লিফ)
    "॥": "\u005C\u005C",
    "\u2014": "\u00D1", // em-dash
...
export function convertToUnicode(text: string): string {
  ...
  // লাইব্রেরি ইতোমধ্যে Ò→“ Õ→’ Ñ→— রাউন্ড-ট্রিপ করে; দাঁড়ি-কোড ¥ (U+00A5)
  // লাইব্রেরি অপরিবর্তিত রাখে — তাই শুধু সেটা হাতে বদলাও।
  r = r.replace(/\u005C{2}/g, "॥").replace(/\u005C/g, "॥");
  // দাঁড়ি-রাউন্ড-ট্রিপ: ¥ (U+00A5) সবক্ষেত্রে দাঁড়ি (আমরাই বিজয়-থেকে
  // বাংলাতে এটি প্রি-ম্যাপ করি নি, তাই সন্দেহ নেই)।
  r = r.replace(/\u00A5/g, "।");
  return r;
}
