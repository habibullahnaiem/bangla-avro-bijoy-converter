...
function preMapPunctuation(s: string): string {
  const map: Record<string, string> = {
    ...
    // ৃ-কার (U+09C3) — সুনতন্নী-কোড e„ (U+0065 U+201E)। লাইব্রেরি শব্দ-শেষে/
    // একক ৃ-কারে ভুলে U+2026 (এলিপসিস) এমিট করে — এখানেই e„-তে বসানো হয,
...
function convertToBijoyRaw(seg: string): string {
  return restoreLibArtifacts(
    ...
    .replace(/\uE001/g, "\u005C\u005C")
    // সুনতন্নী-এ ে-কারের শব্দ-শুরুর নেটিভ কোড \u2020 (†) — বযবহারকারী-নিশ্চিত।
    // রান-ভিত্তিক পুনরূৎপাদনে (restoreCleanUnicode) কনটেক্সট হারায়ে \u2021 পড়লে
